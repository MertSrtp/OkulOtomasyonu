...
namespace OkulOtomasyonu
{
    public partial class Sinavlar : Form
    {
        ...
        {
            int selectedID = Convert.ToInt32(dataGridView1.SelectedRows[0].Cells["SinavID"].Value);
            string query = "DELETE FROM Sinav WHERE SinavID = @SinavID";
            SqlConnection baglanti = new SqlConnection(connectionString);
            baglanti.Open();
            SqlCommand command = new SqlCommand(query, baglanti);
            command.Parameters.AddWithValue("@SinavID", selectedID);
        ...
        {
            using(Sinav_Ekle sinav_Ekle = new Sinav_Ekle())
            {
                sinav_Ekle.ShowDialog();
            }
        }
        ...
        private void button6_Click(object sender, EventArgs e)
        {
                ...
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    try
                    {
                        ...
                        using (SqlCommand cmd = new SqlCommand(query, conn))
                        {
                            ...
                            if (dt.Rows.Count == 0)
                            {
                                MessageBox.Show("Bu sınav koduna ait sınav bulunamadı.", "Bilgi", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            }
                        }
                    }
                    catch (Exception ex)
            ...
            {
                MessageBox.Show("Lütfen bir Sınav Kodu giriniz.", "Uyarı", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
        }

        private void button3_Click(object sender, EventArgs e)
        {
            using(SinavGuncelle sinavGuncelle = new SinavGuncelle())
            {
                sinavGuncelle.ShowDialog();
            }
        }
    }
}
