...
namespace OkulOtomasyonu
{
    public partial class Personel : Form
    {
        ...
        {
            int selectedID = Convert.ToInt32(dataGridView1.SelectedRows[0].Cells["PersonelID"].Value);
            string query = "DELETE FROM Personel WHERE PersonelID = @PersonelID";
            SqlConnection baglanti = new SqlConnection(connectionString);
            baglanti.Open();
            SqlCommand command = new SqlCommand(query, baglanti);
            command.Parameters.AddWithValue("@PersonelID", selectedID);
        ...
        {
            using(Personel_Ekle ekle = new Personel_Ekle())
            {
                ekle.ShowDialog();
        ...
        {
            using (Personel_Guncelle guncelle = new Personel_Guncelle())
            {
                guncelle.ShowDialog();
            }
        }
        ...
        private void button6_Click(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(textBox1.Text))
            {
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    try
                    {
                        ...
                        // Personel ID'ye göre arama yapan SQL sorgusu
                        ...
                        using (SqlCommand cmd = new SqlCommand(query, conn))
                        {
                            ...
                            if (dt.Rows.Count == 0)
                            {
                                MessageBox.Show("Bu Personel ID'ye ait kayıt bulunamadı.", "Bilgi", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            }
                        }
                    }
                    catch (Exception ex)
            ...
            {
                MessageBox.Show("Lütfen bir Personel ID giriniz.", "Uyarı", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
        }
    }
}
